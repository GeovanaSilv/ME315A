---
title: "Lista05_Oliveira"
author: "Geovana Oliveira Barros Silva- 175367"
format: pdf
editor: source
---

# Conexão de R com SQL  e leitura do banco de dados
```{r}
library(DBI)
library(RSQLite)
...
SELECT  ((1.0 * ACRES * 20) / 100) + ACRES FROM PRESERVE 
...
SELECT PNAME, 1.0* ACRES, ACRES/3.0  FROM PRESERVE
...
```{sql connection=con}
SELECT PNAME,FEE, (FEE+50.00)/2 FROM PRESERVE
...
```{sql connection=con}
SELECT ENO, ENAME, SALARY AS OLDSALARY,((SALARY * 10.00)/100) + SALARY AS NEWSALARY FROM EMPLOYEE
ORDER BY NEWSALARY 
